...
void LocationList::move_location(int original_index, int new_index) {
    if (new_index < count) {
        ...
        // if new index is less than original, new index is above original,
        // thus, the list must shift down
        if (new_index < original_index) {
            for (int i = original_index; i > new_index; i--) {
                locations[i] = locations[i - 1];
            }

        // else if new index is greater than the original, new index is below the original,
        // thus, the list must shift up
        } else {
